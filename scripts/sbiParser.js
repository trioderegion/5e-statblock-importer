...
export class sbiParser {
  ...
  static async setMannerAsync(lines, actor) {
    const startText = "manner";
    const line = lines.find( line => line.toLowerCase().startsWith(startText));

    if(line) {
      const foundLine = line.slice(startText.length).trim();

      await actor.update({[game.syb5e.CONFIG.PATHS.manner]: foundLine});
      sbiUtils.remove(lines, line);
    }
  }
  ...
  static async setEquipmentAsync(lines, actor) {
    const startText = "equipment";
    const line = lines.find( line => line.toLowerCase().startsWith(startText));

    if(line) {
      const foundLine = line.slice(startText.length).trim();

      //TODO what to do with equipment line?
      //await actor.update({[game.syb5e.CONFIG.PATHS.shadow]: foundLine});
